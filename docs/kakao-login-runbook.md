...
**✅ 이 런북이 남겼던 구멍은 전부 닫혔다 (2026-09-11 ~ 09-17)**

| 항목 | 상태 |
|---|---|
| ~~**네이티브 앱 키의 `aud`**~~ | ✅ **닫힘 (2026-09-11)** — RN SDK(`@react-native-kakao/user`)로 **실기기 로그인 성공**. `allowed-audiences`에 네이티브 앱 키를 **추가**(목록이라 교체가 아니었다)해 통과했다. `security-spec.md` **L-7 완전 종결** |
| ~~**이메일 미동의 안내 UX**~~ | ✅ **닫힘 (2026-09-17)** — 콘솔에서 이메일을 **필수 동의**로 설정하면 거부 경로가 *로그인 취소*와 합쳐져 **안내 문구가 필요 없다.** `security-spec.md` **L-9 종결**. ⚠️ 단 **A-1의 서버 측 거부는 유지**한다(동의 철회 시 도달) |
| ~~**실기기 redirect URI**~~ | ✅ **해당 없음** — 네이티브 SDK 방식은 **redirect URI가 개입하지 않는다.** 이 항목은 브라우저 OIDC 대안을 전제한 것이었고 그 대안은 폐기됐다 |
| **HTTPS / 실서버** | 열려 있다 — 다만 **카카오 로그인과는 무관**해졌다(위 항목). 배포 자체의 요구사항으로 4-INF에 남는다 |

**남은 것은 서명 키 등록뿐이다.** 카카오는 *APK에 실제로 서명한 키*의 해시로 앱을 식별하는데,
현재 등록된 것은 로컬 `debug.keystore` 하나다.
...
> 💡 **이 런북은 여전히 유효하다.** 웹 플로우는 앱 없이 서버 경로만 떼어 검증하는 수단이라,
> 앱 쪽이 의심스러울 때 **서버는 정상임을 증명하는 대조군**으로 계속 쓸 수 있다.
> `allowed-audiences`에 REST API 키를 **남겨 둔 이유**가 이것이다.
