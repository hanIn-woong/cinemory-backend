...
> **배포 시점·구성은 `CineMory_기획노트.md` 4-INF에 정리돼 있다** (2026-08-27 상의).
> 요지 — 배포 트리거는 날짜가 아니라 **M2에서 카카오 로그인을 실기기에 붙이는 시점**(대략 9월 중순)이고,
> 구성은 **A안(단일 인스턴스에 Spring Boot + MySQL 동거)** 을 채택한다.
>
> ⚠️ **L-10·L-11은 배포를 기다리지 말고 8월 말에 처리한다.** 아래 "처리 시점" 열 참고.

| # | 항목 | 이유 | 처리 시점 |
|---|---|---|---|
| L-9 | **카카오 이메일 필수 동의의 UX** | ⚠️ **사실상 도달 불가로 확인 (2026-09-11)** — 콘솔에서 이메일을 **필수 동의**로 설정하면 사용자가 거부할 방법이 *로그인 취소*뿐이라, 앱에서는 **취소와 같은 경로**로 끝난다(별도 안내 문구가 필요 없다). 그래서 이 케이스는 M2 검증에서 **실행하지 못한 것이 아니라 재현할 수 없었다.** ⚠️ **그래도 A-1의 서버 측 거부는 유지한다** — 사용자가 카카오 계정 설정에서 **동의를 철회**하거나 콘솔의 동의항목이 선택으로 바뀌면 그때 도달한다. **방어 코드를 지우지 말 것** | 종결(안내 문구 불요) |
| L-10 | **`jwt.secret` 환경변수 주입** | 현재 `application-secret.yml`. 배포 시 환경변수로 전환 | ⚠️ **8월 말 (선행)** — 지금은 설정 한 줄인데, 배포 중에 만나면 *기동은 실패하고 로그는 다른 얘기를 하는* 상황이 된다. 배포하지 않고도 지금 검증된다. A→C(컨테이너) 전환 시 요구되는 12-factor 설정의 출발점이기도 하다 |
| L-11 | **시간대 고정** | `Clock.systemDefaultZone()`과 JPA Auditing이 모두 JVM 기본 시간대를 따른다. `TZ` 또는 `-Duser.timezone`으로 **한 곳에서** 맞출 것 | ⚠️ **8월 말 (선행)** — 로컬은 KST, 클라우드 기본은 대개 **UTC**다. 9시간 어긋난 `expires_at`은 **"로그인하자마자 만료"** 같은 엉뚱한 증상으로 나와 원인 추적이 오래 걸린다. 배포 후에 발견하면 인증 경로 전체를 의심하게 된다 |
| L-13 | ⚠️ **`user.profile_image` 저장 위치 결정** (2026-08-27 신설) | `varchar(500)` 컬럼은 있으나 **지금 채우는 업로드 경로가 없다**(카카오 프로필 URL 형태). 코드에 로컬 파일 I/O가 0건이라(`MultipartFile`·`Files.`·`new File(` 전부 없음) 현재는 어디에도 결합돼 있지 않다. **M2에서 "프로필 사진 업로드"를 붙이는 순간 갈린다** — 인스턴스 로컬 디스크에 두면 컨테이너 재시작·인스턴스 교체마다 사라진다. **처음부터 오브젝트 스토리지나 외부 URL로 가면 비용이 0**이고, 로컬에 두면 나중에 코드 + 데이터 이전 + URL 일괄 갱신이 붙는다. 포스터를 경로만 저장한 판단(`tmdb-sync-spec.md` 6-3)을 여기에도 적용한다 | **M2에서 업로드 기능 설계할 때** (기능을 붙이기 전) |
| L-12 | ~~**Springdoc 운영 노출 차단**~~ (controller-layer-spec.md 5-0-G / 5-7 D) | ✅ **완료** (2026-08-11) — `application-prod.yml` 신설, `springdoc.api-docs.enabled=false`/`springdoc.swagger-ui.enabled=false`. `SPRING_PROFILES_ACTIVE=prod` 실기동으로 `/v3/api-docs`·`/swagger-ui/index.html` 404, 일반 API는 200, 자동배선 경고 로그 자체가 안 찍힘(설정만 숨긴 게 아니라 자동구성 자체가 꺼짐)을 확인 | ✅ 완료 |
...
| 날짜 | 내용 |
|---|---|
| 2026-09-11 | **✅ L-7 완전 종결 — 네이티브 앱 키 `aud` 검증 완료 (프론트 B-1에서 해소).** 2026-08-27의 부분 종결이 남긴 유일한 구멍이 *"통과한 `aud`는 REST API 키이고 네이티브 앱 키는 미검증"* 이었는데, M2 프론트에서 카카오 SDK를 붙여 **실기기 로그인이 성공**하면서 닫혔다. `allowed-audiences`의 네이티브 앱 키가 매칭되지 않았다면 `INVALID_OAUTH_TOKEN`이 났을 것이므로 **로그인 성공 자체가 `aud` 검증**이다. `KakaoOAuthProperties`를 *"플랫폼에 따라 `aud`가 다르다"* 는 예상 위에 **목록으로 설계해 둔 것이 두 번째로 값을 했다** — 교체가 아니라 한 줄 추가로 끝났다. 함께 **L-9를 종결**했다 — 이메일을 **필수 동의**로 설정하면 거부 경로가 *로그인 취소*와 합쳐져 **안내 문구가 필요 없어진다.** 다만 **A-1의 서버 측 거부는 유지**한다(동의 철회·콘솔 설정 변경 시 도달). ⚠️ **미등록 서명 키 2종이 남는다** — EAS 키스토어(팀 배포)와 **Play 앱 서명 키(M5)**. 후자는 빠뜨리면 스토어 배포 후에만 깨지는 종류라 M5 체크리스트 항목이다. **이 갱신은 리포 경계를 거슬러 올라온 첫 사례다** — 그동안 규칙(2026-09-02)은 *"백엔드 계약은 백엔드 리포에 쓴다"* 였는데, **백엔드 미결이 프론트 작업으로 닫히는 방향**에는 경로가 없어 L-7이 6일간 낡은 상태로 남아 있었다. 프론트 세션이 백엔드 항목을 해소하면 여기로 돌려보낸다 |
| 2026-08-27 | **✅ 카카오 실토큰 검증 성공 — L-7 부분 종결, L-14 신설.** 런북 절차로 실토큰이 **서명·`iss`·`aud`·`nonce` 4종을 모두 통과**하고 계정 생성·JWT 발급까지 확인됐다. **실행 중 드러난 것 4건을 런북에 반영했다.** ① **`client_secret` 누락 — 런북 초안의 실수.** 클라이언트 시크릿을 활성화한 앱은 토큰 교환에 이 값이 없으면 `KOE010`으로 **401**이다. ② **`aud`가 REST API 키였다** — 예상대로였고(`KakaoOAuthProperties`가 *"플랫폼에 따라 `aud`가 다르다"* 를 미리 예상해 **목록으로 설계해 둔 것이 값을 했다**), `allowed-audiences`에 추가하니 통과했다. ③ ⚠️ **L-14 — `INVALID_NONCE`의 두 원인이 구분되지 않아 진단이 막혔다.** `consumeOrThrow`가 먼저 nonce를 소비하는 탓에 값 불일치도 만료처럼 보인다. `id_token`을 디코딩해 클레임을 직접 대조하는 절차를 런북에 넣었다. ④ **PowerShell `Invoke-RestMethod`가 4xx 본문을 삼킨다** — `try/catch` + `ErrorDetails`(비면 응답 스트림 직접 읽기)가 없으면 *"401"* 만 보이고 `ErrorCode`를 못 본다. 성공 출력을 `try` 안에 두면 `null` 프로퍼티 접근 예외가 `catch`로 흘러 **아무것도 출력되지 않는 침묵**이 발생하는 것도 확인. **또 하나의 교훈 — nonce TTL 5분은 앱 기준이라 수동 브라우저 왕복에는 빠듯하다.** 검증용으로 임시 연장이 필요했고, 최종 성공은 1~4단계를 한 스크립트로 이어 붙여 **15초**에 끝냈다. ⚠️ **`nonce-ttl`은 `PT5M`으로 되돌릴 것** |
| 2026-08-27 | **`docs/kakao-login-runbook.md` 신설 — 카카오 실토큰 로컬 검증 절차.** 미검증 E2E 2건 중 **카카오 실토큰**을 앱 없이 닫기 위한 절차다(브라우저 웹 플로우로 `id_token`을 받아 `POST /api/auth/oauth/kakao`에 넣는다). **작성 동기는 순서 문제다** — 이 검증이 지금 구조상 **배포 트리거와 정확히 같은 시점에 온다**(기획노트 4-INF). *"카카오 로그인 처음 붙이기 + 실토큰 처음 검증 + 실서버 처음 올리기 + redirect URI 처음 등록"* 이 겹치면 실패 시 **어느 층인지 가려낼 수 없다.** 로컬에서 한 번 통과시키면 배포 시점 미지수가 **넷에서 둘(redirect URI·HTTPS)로** 준다. **런북에 반영한 설계상 함정 3건** — ① **OIDC 활성화가 꺼져 있으면 `id_token`이 아예 오지 않는다**(에러가 아니라 *응답에 필드가 없는* 형태라 알아채기 어렵다). `scope=openid` 누락도 같은 증상이다. ② **`nonce` TTL 5분**(`nonce-ttl: PT5M`)이라 브라우저 왕복을 그 안에 끝내야 한다 — 수동 절차에서만 드러나는 제약이다. ③ **웹 플로우는 `aud`가 REST API 키**라 `allowed-audiences`에 추가해야 한다. `KakaoOAuthProperties`가 *"플랫폼에 따라 `aud`가 다르다"* 를 예상해 **목록으로 설계해 둔 것이 여기서 값을 했다** — 교체가 아니라 추가라, 런북용 키와 앱용 네이티브 키를 나란히 두면 같은 설정으로 둘 다 돈다. **닫지 못하는 것도 명시** — 네이티브 앱 키의 `aud`, 실기기 redirect URI·HTTPS, L-9 안내 UX |
| 2026-08-27 | **S-11 "배포 전 반드시 처리할 것"에 처리 시점 열 추가 + L-13 신설.** 인프라·배포 구성 상의(`CineMory_기획노트.md` **4-INF**) 결과 반영. **표가 그동안 "배포 전"이라는 한 덩어리였는데, 실제로는 세 부류였다.** ① **L-10·L-11은 배포를 기다릴 이유가 없다** — 배포하지 않고도 지금 검증되고, 배포 중에 만나면 **진단이 가장 어려운 유형**이다. L-10은 기동 실패인데 로그가 다른 곳을 가리키고, L-11은 로컬 KST / 클라우드 UTC 차이가 *"로그인하자마자 만료"* 같은 엉뚱한 증상으로 나와 인증 경로 전체를 의심하게 만든다. → **8월 말 선행**으로 옮겼다. ② **L-9는 카카오 로그인을 붙이는 시점**에 함께 처리한다(그게 곧 배포 트리거다 — 실기기에서 `localhost`는 폰 자신을 가리키고, redirect URI는 LAN IP로 우회되지 않는다). ③ **L-13 신설 — `user.profile_image` 저장 위치.** `varchar(500)` 컬럼은 있으나 채우는 업로드 경로가 없고, 현재 코드에 **로컬 파일 I/O가 0건**이라(`MultipartFile`·`Files.`·`new File(` 전부 없음) 어디에도 결합돼 있지 않다. **M2에서 업로드를 붙이는 순간 갈린다** — 로컬 디스크에 두면 컨테이너 재시작·인스턴스 교체마다 사라지고, 처음부터 오브젝트 스토리지/외부 URL이면 비용이 0이다. 포스터를 경로만 저장한 판단(`tmdb-sync-spec.md` 6-3)과 같은 결정이며, **배포 구성 A→B/C 전환 비용을 좌우하는 단일 최대 변수**라 기능 설계 전에 정해야 한다. **L-12는 열 추가에 따라 "✅ 완료" 표기만 보정**(내용 변경 없음) |
| 2026-08-11 | **L-12(Springdoc 운영 노출 차단) 완료** — `controller-layer-spec.md` 5-7 D 진행 중 처리. `application-prod.yml` 신설로 `springdoc.api-docs.enabled=false`/`springdoc.swagger-ui.enabled=false`, `SPRING_PROFILES_ACTIVE=prod` 실기동으로 `/v3/api-docs`·`/swagger-ui/index.html` 404·일반 API 200을 확인. 상세는 `controller-layer-spec.md` 5-7 D 변경 이력 참고 |
