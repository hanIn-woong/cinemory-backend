...
| L-12 | ~~**Springdoc 운영 노출 차단**~~ (controller-layer-spec.md 5-0-G / 5-7 D) | ✅ **완료** (2026-08-11) — `application-prod.yml` 신설, `springdoc.api-docs.enabled=false`/`springdoc.swagger-ui.enabled=false`. `SPRING_PROFILES_ACTIVE=prod` 실기동으로 `/v3/api-docs`·`/swagger-ui/index.html` 404, 일반 API는 200, 자동배선 경고 로그 자체가 안 찍힘(설정만 숨긴 게 아니라 자동구성 자체가 꺼짐)을 확인 |
