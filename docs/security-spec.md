...
| # | 항목 | 내용 |
|---|---|---|
| L-7 | ~~**카카오 실토큰 E2E**~~ | ✅ **부분 종결 (2026-08-27)** — `docs/kakao-login-runbook.md`의 웹 플로우로 **실토큰이 서명·`iss`·`aud`·`nonce` 4종 검증을 모두 통과**하고 계정 생성·JWT 발급까지 확인됐다. **단위 테스트가 우리가 정한 값끼리 대조한다는 한계가 실제로 해소된 것은 이번이 처음이다.** ⚠️ **다만 통과한 `aud`는 REST API 키다.** 네이티브 SDK로 로그인하면 `aud`가 **네이티브 앱 키로 바뀌므로 그 값은 여전히 미검증**이다 — `allowed-audiences`가 목록이라 **한 줄 추가**로 끝나지만, 모르면 M2에서 `INVALID_OAUTH_TOKEN`을 그대로 다시 만난다. 선행: 콘솔 플랫폼 등록(Android 키 해시 / iOS 번들 ID) |
...
| # | 항목 | 이유 | 처리 시점 |
|---|---|---|---|
| L-9 | **카카오 이메일 필수 동의의 UX** | 사용자가 동의를 거부하면 **로그인 자체가 불가**하다(A-1). 안내 문구가 필요하다 | 배포 시 (카카오 로그인 붙일 때 함께) |
