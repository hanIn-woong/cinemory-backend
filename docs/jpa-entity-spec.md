...
- **`update(watchDate, watchType, placeDetail, ottPlatform, rating, note)`** (2026-09-02 추가) —
  ...
  - ⚠️ **setter를 열지 않는다.** `validateRating()`이 `@Builder` 생성자에서만 불리고 있으므로,
    ...
    `updateWatchRecord`가 이 메서드만 호출하도록 강제한다.
  - **`movie`와 `representative`는 파라미터에 넣지 않는다.** 전자는 바뀌면 다른 기록이고,
    후자는 `markAsRepresentative()`/`unmarkAsRepresentative()`가 전담한다 — 두 경로가 같은
