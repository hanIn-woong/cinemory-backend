...
| 6-10 | **적재 규모 확대 — 계층형 수집 전략** (4,609편 → 12,750편). 전량 미러링 기각, 프로필 v2 | ✅ **확정** (2026-09-11) → **실행·실측 완료** (2026-09-12, 잔여 #29 종결 — 결과는 **6-7-c**) |
...
> ⚠️ **프로필 표와 목표 규모는 6-10(2026-09-11)으로 대체됐다.** 아래 4개 프로필은 4,609편을
> 만든 v1이며, 기록으로 남긴다. **파라미터 pass-through 원칙과 `region=KR` 철회는 그대로 유효**하다.
> 6-10이 뒤집은 것은 *"임계값(30/300/100)이 조정 대상"* 이라는 진단 하나다 —
> `sort_by=*.desc`와 조합된 하한은 아무것도 거르지 않으며, 실질 커트라인은 `pages`였다.
...
## 6-7-c. 프로필 v2 실측 결과 — 12,750편 (2026-09-12)

`movie-seed-runbook.md` v2에 따라 6-10 프로필 4종(33회)을 실행했다. 실행 시간은
**00:43:56 ~ 01:28:00, 약 44분** — 예상(약 2시간)보다 훨씬 짧았다(429 0건, 재시도 없이
순차 진행됐기 때문).

### 적재 규모

| 테이블 | 4,609편(v1) | **12,750편(v2)** | 비고 |
|---|---|---|---|
| `movie` | 4,609 | **12,750** | 목표 1.8만~2만 미달 — 아래 "수확 재판정" 참고 |
| `movie_actor` | 186,717 | **446,932** | 영화당 평균 35.1(← 40.5, 소외 장르 캐스팅 규모가 작은 편) |
| `person` | 113,909 | **231,931** | |
| `box_office_record` | 140 | **280** | 미매칭 58건(20.7%) — 잔여 #10 참고 |

### 프로필별 기여 편수 (`SeedResult`, 잔여 #29의 근거)

| 프로필 | 호출 | 순 신규(`matched`) | `alreadyExists` |
|---|---|---|---|
| 박스오피스 역방향 | 1 | 11 | 2 |
| ① 한국 영화 | 1 | 732 | 837 |
| ② 장르 보강 | 19 | **1,579** | — |
| ③ 최근작(2015~2026) | 12 | **2,288** | — |
| ④ 전역 인지도 | 1 | 3,499 | 4,501 |
| **합계** | 34 | **8,109** | |

`stoppedByRateLimit`는 34회 전부 `false` — 429 없이 완주했다.

### 확인사항 ① — `vote_count` 2,000~6,000 구간 (프로필 ④ `pages=400` 검증)
...
| 미드소마 | 8,342 |
| 살인의 추억 | 4,767 |
| 올드보이(동명 별도 레코드) | 2,186 |
| 버닝(동명 별도 레코드) | 1,872 · 362 |

`vote_count BETWEEN 2000 AND 6000` = **1,940편**. 6-10이 지목한 「올드보이」(약 4,500표대
기록)가 실제로 들어왔다 — 6-5의 `pages=100` 커트라인 문제가 `pages=400`으로 해소됐다는
직접 증거다. 「올드보이」·「버닝」이 각 2건씩 잡힌 것은 TMDB에 동명 이작(리메이크/별도
등록)이 실제로 존재하기 때문으로 보이며, 별도 조치는 없다(잔여 등록 없음 — 중복 방지는
`tmdbId` 기준이라 정상 동작).

### 확인사항 ② — 한국 영화 (`minVotes` 30→10, `pages` 50→150)
...
한국 영화가 843 → **1,595편(89% 증가)**. 하한을 낮춘 효과가 명확하다 — 다만 프로필①
자체의 순 기여(732)가 늘어난 843 전부를 설명하지는 않는다(box-office 역방향·4-7 재매칭도
한국 영화를 끌어들인다).

### 확인사항 ③ — 장르별 편수 (프로필 ②의 존재 이유)
...
| 음악 | 430 |
| 역사 | 653 |
| … | (액션 3,035 · 드라마 5,532까지 상승) |

**19종 전부 300편을 넘겼다** — 프로필②가 각 장르 `pages=15`(최대 300편)로 하한을 보장한
설계 그대로 작동했다. `SeedResult`를 보면 이미 baseline(4,609편, 전역 상위권 위주)에
포함돼 있던 액션·드라마·코미디 등은 `matched=0`(이미 300 전부 존재)이었고, **다큐(273)·
TV 영화(271)·서부(248)·음악(205)** 처럼 소외 장르일수록 순 기여가 컸다 — 6-10이 예측한
그대로다.

### 잔여 #28 재확인 — `character_name` 절단, 여전히 3건 수준

절단(`truncate()` 시그니처 — 길이 255 + `...` 접미사)이 발생한 **영화는 3편**
(`movie_id` 1546·2956·12421, actor 행 기준 7건). v14 확장(255) 적용 후에도 절단 0으로
떨어지지 않는다는 6-8-27 관측이 재확인됐다 — 비율이 여전히 극히 낮아(12,750편 중 3편,
0.024%) **잔여 #28의 "확장 보류" 판정을 유지**한다.

`movie.title`(최대 78) · `movie.original_title`(최대 127) · `movie.overview`(최대 994/1000) ·
`person.name`(최대 46) — 전부 상한 대비 여유 있고 절단 0건.

### 수확 재판정 — 1.8만~2만 추정 대비 12,750편(약 65~70%)

6-10의 "합계 21,500 요청 → 중복 제거 후 약 1.8만~2만편" 추정이 **실측보다 높았다.**
원인은 **프로필 간 겹침이 예상보다 컸다**는 것이다 — 특히 ④(전역, 8,000요청)가
`alreadyExists=4,501`(56%)이었고, ②(장르, 5,700요청)도 액션·드라마 등 대형 장르에서
`alreadyExists=300`(전량)이 반복됐다. 이는 **baseline(v1, 4,609편)이 이미 전역
상위권 위주였기 때문**이다(6-8-27 v1 회고 — `pages=100`이었지만 그 안에서도
`vote_count.desc` 상위는 장르 불문 겹친다). **재작업 판단은 없음** — 목표는 "정확히
2만편"이 아니라 "장르별 표본 확보"(M3 입력)였고 확인사항 ③에서 그 목표는 달성됐다.
12,750편으로도 v1(4,609편) 대비 **2.77배**이며 서부·다큐 등 최소 장르도 300편을 넘겼다.

### ⚠️ 계측 정정 3건 — 6-10의 예측 모델이 틀렸다

결과는 목적을 충족했지만 **예측에 쓴 수치 세 개가 전부 빗나갔다.** 다음 확대 때 같은
실수를 반복하지 않도록 근거까지 남긴다.

| # | 6-10 예측 | **실측** | 틀린 이유 |
|---|---|---|---|
| 1 | 소요 **약 2시간** | **44분 4초** | resync 처리율을 시드에 그대로 외삽했다 |
| 2 | 프로필 ④ 단일 호출 **약 1시간** | **17분 9초** | 위와 같은 원인 |
| 3 | 중복 손실 **7~16%** | **59.6%** | baseline이 이미 전역 상위권을 덮고 있었다 |

#### ① ② 처리율 — `resync`와 시드는 같은 단위가 아니다

6-10은 v14 resync 실측(**분당 155.5건**)을 그대로 썼다. **이 값은 "영화 1편 = TMDB 왕복 1회
+ 매핑 4종 삭제·재삽입"의 처리율**인데, 시드에서는 **중복 59.6%가 `existsByTmdbId` 사전
필터에 걸려 detail 호출조차 하지 않는다.** 즉 20,082 슬롯 중 실제로 왕복이 발생한 건
8,109건뿐이었다.

```
반환 슬롯 20,082 / 44분  = 분당 456 슬롯   ← 사전 필터 덕에 대부분이 거의 공짜
순 신규  8,109 / 44분  = 분당 184건     ← 실제 왕복이 일어난 건들
```

**시드 처리율(184/분)이 resync(155.5/분)보다 오히려 높다.** 신규 INSERT가 resync의
"벌크 삭제 → 재삽입 + dirty checking"보다 가볍기 때문으로 보인다. 앞으로
**시드 예측에는 `분당 184건 × 예상 순 신규 편수`를, resync 예측에만 155.5를 쓴다.**

#### ③ 중복률 — 6-7이 경고한 표본 편향의 재발

6-10은 21,500 요청에서 1.8만~2만편을 기대해 **손실 7~16%** 를 가정했다. 실제는 **59.6%**
(20,082 슬롯 중 `alreadyExists` 11,973)다. 원인은 **baseline 4,641편의 성격**이다 — v1이
`vote_count.desc` 상위 위주로 뽑혔기 때문에, 같은 축을 쓰는 프로필 ④(전역)와 ②(장르)가
구조적으로 그 위를 덮는다.

| 프로필 | 정렬축 | `alreadyExists` 비율 |
|---|---|---|
| ④ 전역 인지도 | `vote_count.desc` | **56.3%** (4,501 / 8,000) |
| ② 장르 보강 | `vote_count.desc` | **72.3%** (4,121 / 5,700) |
| ③ 최근작 | `popularity.desc` | 52.3% (2,512 / 4,800) |

⚠️ **이 59.6%를 그대로 다음에 쓰면 안 된다.** 이 값은 *"baseline이 이미 같은 정렬축의
상위를 덮고 있을 때"* 의 값이다 — 6-7이 60편 표본에서 경고했던 **표본 편향과 같은 유형의
함정**이고, 정렬축이 다른 프로필을 새로 추가하면 다시 내려간다.

#### 파급 — `resync` 재추정

`resync`는 사전 필터가 없어 **전 건이 왕복**이므로 155.5/분이 그대로 유효하다.

```
12,750편 ÷ 155.5 = 약 1시간 22분      (6-10의 "2만편 → 약 2시간 10분" 대체)
```

**잔여 #31(증분 동기화)의 시급성이 낮아진다** — `resync(fromId, limit)` 분할 실행으로
충분히 감당되는 범위다.

> **전량 미러링 기각 판단은 바뀌지 않는다.** 시드 처리율(184/분)로 다시 계산해도
> 140만편은 **약 5.3일**이고, 6-10이 든 기각 사유 3건(resync 불성립 · M3 인프라 선점 ·
> 품질 저하) 중 처리율에 기대는 건 하나뿐이다.

### ⚠️ `seed.log`가 344MB로 불어났다

`SeedResult` 분리(잔여 #26) 덕에 계측 자체는 무사했으나, 서버 로그가 **343,795,238바이트**가
됐다. SQL/DEBUG 레벨이 켜져 있을 가능성이 높다. 이 크기면 절단·폴백 `grep` 검증이 느리고
다음 확대에서 더 커진다 — **잔여 #32**로 등록한다.

### 잔여 #10 — 표본이 커졌으나 판정은 유보

`box_office_record` 280건 중 58건(20.7%) 미매칭 — 6-7-b의 90.7%(140건 기준)보다
낮아 보이지만, 이번 표본은 **여러 세션에 걸쳐 누적된 레코드**(일부는 아직 4-7 재매칭
배치를 거치지 않았을 수 있음)라 **단순 비교는 부적절**하다. 재판정하려면 재매칭 배치
실행 후 재확인이 필요 — 새 잔여로 등록하지 않고 다음 박스오피스 점검 때 자연히
같이 본다.
...
## 6-10. 적재 규모 확대 — 계층형 수집 전략 (✅ 확정 2026-09-11)

6-7-b의 4,609편으로 운영해 보니 **명작이 빠진다.** 목표를 약 **2만편**으로 올리되,
"전량 미러링"은 명시적으로 기각하고 **계층 구조**로 확정한다.

### ⚠️ 진짜 커트라인은 `vote_count.gte`가 아니라 `pages`다

6-5가 *"인지도 축은 `vote_count`다"* 라고 정하면서 **임계값(30 / 300 / 100)을 조정 대상으로
지목했는데, 그 진단이 절반만 맞았다.**

프로필 2는 `minVotes=300` + `sort_by=vote_count.desc` + `pages=100`이다.
**정렬이 이미 표 많은 순 내림차순이므로, 2,000번째 영화의 `vote_count`는 300보다 한참 높다.**
즉 **하한 300은 단 한 편도 거르지 않는다.** 실질 커트라인은 `pages × 20 = 상위 2,000위`이고,
그 경계선은 `vote_count` 5,000~6,000 언저리로 추정된다.

| 작품 | 대략 `vote_count` | 현재 프로필 2(상위 2,000)에 드는가 |
|---|---|---|
| 미드소마 | 약 7,000 | 아슬아슬하게 들어옴 |
| **올드보이** | **약 4,500** | **빠진다** |

**`sort_by`가 `*.desc`인 프로필에서 하한값을 만지는 것은 아무 효과가 없다. `pages`를 올려야 한다.**
반대로 프로필 3은 `sort_by=popularity.desc`라 정렬축과 필터축이 달라 **하한이 실제로 작동한다.**
같은 파라미터가 프로필마다 의미가 다르다는 점을 아래 표에 명시한다.

> 6-5의 프로필 표는 이 절로 **대체된다.** `region=KR` 철회와 pass-through 원칙은 그대로 유효하다.

### 계층 구조 — Tier 2가 규모 문제를 대신 푼다

| Tier | 수단 | 시점 | 커버리지 |
|---|---|---|---|
| **1. 사전 배치** | discover 프로필 4종 + 박스오피스 역방향 (6-5) | 관리자 실행 | **약 2만편** |
| **2. 온디맨드** | 검색 `suggestions` → `POST /api/movies/sync` (6-5 온디맨드 절, 6-8) | 사용자가 선택할 때 | **TMDB 전량(약 140만)** |
| **3. 증분 갱신** | `POST /api/admin/movies/resync` (6-9) | 주기 실행 | 적재분 전체 |

**Tier 2는 이미 구현돼 있다.** 이 절이 새로 정하는 것은 Tier 1의 규모뿐이다.
*"사용자가 기록하려는 영화가 DB에 없다"* 는 문제는 **Tier 1을 키워서가 아니라 Tier 2가 푼다** —
저장은 2만편인데 체감 커버리지는 140만편이 된다. Tier 1의 목적은 커버리지가 아니라
**검색 첫 화면 품질·박스오피스 매칭·M3 추천 후보군**이다.

### 전량 미러링을 기각한다

6-7-b·v14 resync의 **실측치(4,587건 / 29분 30초 = 분당 155.5건)** 를 그대로 외삽했다.

| 항목 | 4,609편 (현재) | **2만편 (확정)** | 전량 (약 140만) |
|---|---|---|---|
| 최초 적재 소요 | — | **약 2시간** | **약 6.3일** |
| `resync` 1회 | 29분 30초 | **약 2시간 10분** | **약 6.3일** |
| `person` 행 | 113,909 | 약 38만 | 약 300만 |
| `movie_actor` 행 | 186,717 | 약 81만 | 약 5,600만 |
| 검색(`LIKE` 풀스캔) | 수 ms | 수십 ms | 1~2초 — 외부 엔진 필수 |
| M3 임베딩(768dim 가정) | 14MB | **60MB — 인메모리 가능** | 4.2GB — 벡터 DB 전제 |

> ⚠️ **정오 (2026-09-12)** — 위 "2만편 / 약 2시간"은 resync 처리율을 시드에 외삽한 값이라 틀렸다. **실측은 12,750편 / 44분**이며 `resync` 1회는 **약 1시간 22분**이다. 전량(140만편)도 시드 처리율로 다시 계산하면 6.3일이 아니라 **약 5.3일** — **기각 결론은 그대로다**(6-7-c 계측 정정).

**기각 사유 3건.**

1. **`resync`가 성립하지 않는다.** 6-9가 *"`resync`는 선택이 아니라 필수"* 라고 확정했다 —
   `vote_average`/`vote_count`가 시간에 따라 변하기 때문이다. **1회 6일짜리 상시 작업은 없다.**
2. **M3 설계를 먼저 못 박게 된다.** 임베딩이 4.2GB면 MySQL 보관이 불가능해져 외부 벡터 DB가
   **전제**가 된다. 추천 설계가 아직 확정되지 않았는데 인프라가 데이터 결정에 끌려가는 순서다.
3. **품질이 나빠진다.** 늘어나는 약 135만편의 대부분이 `vote_count = 0`이다. 검색 결과와 추천
   후보군을 오염시키고, 결국 쿼리마다 하한을 다시 붙이게 된다 — **그러면 안 넣은 것과 같다.**

> **목표를 1만이 아니라 2만으로 잡은 이유** — 프로필 2의 `pages`를 400으로 올리는 것만으로
> 8,000편이 나온다. 1만에 맞추려면 장르 보강(프로필 4)을 빼야 하는데, 그건 M3 입력을 깎는
> 선택이다. **비용 곡선이 2만까지는 평평하다**(시드 2시간, 임베딩 60MB). 1만과 2만 사이에
> 넘는 임계점이 없으므로 굳이 1만에 맞추지 않는다.

### 프로필 재구성 v2

| # | 목적 | 파라미터 | pages | 요청 편수 | 하한이 실제로 작동하는가 |
|---|---|---|---|---|---|
| 1 | **한국 영화** | `lang=ko` · `minVotes=10` · `sortBy=vote_count.desc` | 150 | 3,000 | ❌ 실질 커트라인은 상위 3,000위 |
| 2 | **전역 인지도** | `minVotes=50` · `sortBy=vote_count.desc` | **400** | 8,000 | ❌ 실질 커트라인은 상위 8,000위 |
| 3 | **최근작** | `year=2015~2026` 각각 · `minVotes=30` · `sortBy=popularity.desc` | 20 | 400 × 12 = 4,800 | ✅ 정렬축과 달라 작동 |
| 4 | **장르 보강 (신규)** | `genres=<id>` · `minVotes=30` · `sortBy=vote_count.desc` | 15 | 300 × 19 = 5,700 | ❌ 장르 내 상위 300위 |
| — | 박스오피스 역방향 | (6-5 그대로) | — | 실제 흥행작 | — |
| **합계** | | | | **21,500 요청** | **중복 제거 후 약 1.8만~2만편** |

- **1번 — 하한을 30 → 10으로 내린다.** 6-7-b에서 `minVotes=30`으로 한국 영화가 **843편**밖에
  안 나왔다(목표 1,000 미달). 표가 부족한 것이지 작품이 없는 게 아니다. `pages`도 50 → 150.
- **2번 — `pages`를 100 → 400으로 올린다.** 위에서 본 「올드보이」 구간이 여기서 들어온다.
  ⚠️ **TMDB `/discover` 페이지 상한은 500이다.** 400은 안전하나 **450을 넘기지 말 것.**
- **3번 — 연도 범위를 2021~2025 → 2015~2026으로 넓히고 하한을 100 → 30으로.** 여기서만
  하한이 실제로 작동하며, 2026년작은 아직 표가 안 쌓여 100이면 거의 다 잘린다.
- **4번 — 신설.** 프로필 2가 전역 `vote_count.desc`라 액션·SF·드라마가 상위를 독식한다.
  **다큐멘터리·애니메이션·서부·음악 장르는 장르 내 대표작조차 전역 상위 8,000에 못 든다.**
  「배경」절이 밝힌 대로 **M3 추천 입력이 `movie_genre` 가중치**이므로, 장르별 표본이 비면
  그 장르의 추천 자체가 성립하지 않는다. 장르 19종은 6-1이 이미 `genre`에 적재해 뒀다.

### 실행 순서 — 좁은 프로필부터

6-5는 *"박스오피스가 먼저"* 였으나 그 근거(잔여 #10 실측)는 6-7-b에서 **90.7%로 종결**됐다.
순서 강제가 풀렸으므로 이번엔 **기여도 계측**이 기준이 된다 — 넓은 프로필을 먼저 돌리면
나머지가 전부 `alreadyExists`로 빠져 **프로필별 기여 편수를 알 수 없다.**

```bash
# 1. 한국 영화 (가장 좁고 독립적)
POST /api/admin/movies/seed/discover?pages=150&lang=ko&minVotes=10&sortBy=vote_count.desc
# 2. 장르 보강 — 장르 19종 각각 (2번보다 먼저여야 기여도가 보인다)
POST /api/admin/movies/seed/discover?pages=15&genres=99&minVotes=30&sortBy=vote_count.desc
#    ... genre_id 19개 반복
# 3. 최근작 — 2015 ~ 2026 각각
POST /api/admin/movies/seed/discover?pages=20&year=2015&minVotes=30&sortBy=popularity.desc
# 4. 전역 인지도 (가장 넓다 — 마지막)
POST /api/admin/movies/seed/discover?pages=400&minVotes=50&sortBy=vote_count.desc
```

⚠️ **discover 33회 + 박스오피스 역방향 1회 = 34회 호출.** 소요는 초판에 *"약 2시간"* 이라 적었으나 **실측 44분**이다(6-7-c 계측 정정 ①). 6-7-b에서 `seed.log`를 유실해 `SeedResult` 8건을 확인하지
못한 전례가 있다(잔여 #26). PowerShell `*>`는 **덮어쓰기**이므로 반드시 `*>>`로 이어붙이거나
프로필마다 파일명을 분리할 것.

### 규모 파급 — 재측정이 필요한 것과 아닌 것

| 잔여 | 판정 |
|---|---|
| **#8** `movie_actor` 인덱스 | **재측정 불요.** 6-7-b가 2,375 → 186,717행(**79배**) 동안 `EXPLAIN rows=141` 불변을 확인했다. 4.3배 추가 증가는 판정을 뒤집을 수 없다 |
| **#20** 한글 FULLTEXT | **아직 불요.** 2만행 풀스캔은 수십 ms다. **트리거를 재설정한다 — `movie` 10만행 돌파 또는 검색 p95 300ms 초과** |
| **#18** 다중 인스턴스 | **위험 상승.** 시드가 2시간짜리가 되면서 `AtomicBoolean`(JVM 내 유효)의 한계 노출 구간이 그만큼 길어진다. 판정은 그대로 별도 세션 |
| **#26** 런북 보강 | **선행 조건으로 승격.** 위 ⚠️ 참조 |

### 코드 변경은 한 곳뿐 — `with_genres`

프로필 4를 위해 **`TmdbClient.discoverMovies`에 파라미터 하나만 추가**한다.
나머지 조정값은 전부 기존 pass-through로 URL만 바꾸면 된다(6-5 원칙이 여기서 값을 한다).

```java
// 추가: withGenres — null이면 붙이지 않는다 (queryParamIfPresent, 기존 패턴 그대로)
TmdbDiscoverResponse discoverMovies(int page, String originalLanguage, Integer voteCountGte,
                                    String sortBy, Integer year, String withGenres);
```

⚠️ **`with_genres`는 콤마가 AND, 파이프(`|`)가 OR다.** 프로필 4는 장르를 하나씩 넘기므로
현재는 무관하지만, 나중에 복수 장르를 넘길 때 **콤마로 이으면 교집합이 되어 결과가 급감**한다.

### 검토했으나 기각 — `adult` 필터 폐기 (2026-09-11)

*"성인 영화에도 좋은 작품이 많으니 6-3 ①을 폐기하자"* 는 안이 나왔으나 **전제가 사실과 달라
기각**했다. TMDB 영화 기여 지침서가 `adult` 플래그를 **하드코어 포르노 전용**으로 규정한다.

- 18금 에로틱 영화는 `adult`로 표시하지 **않는다** — 「그레이의 50가지 그림자」·「님포매니악」이
  지침서의 명시적 반례다.
- 하드코어 장면이 있어도 **일반 영화로 개봉되고 평론가 리뷰를 받았으면 예외**다(「9 Songs」 등).
- 포르노 **다큐멘터리**도 일반 영화로 취급한다.

**「올드보이」·「미드소마」는 `adult:false`이고 이미 수집 대상이다.** 이들이 DB에 없다면 원인은
`adult` 필터가 아니라 **프로필 2의 `pages=100` 커트라인**이며, 그것이 바로 이 절이 고치는 문제다.
문제를 잘못 짚으면 **필터를 지워도 그 영화들은 여전히 안 들어온다.**

> **연령 등급으로 통제할 축은 `adult`가 아니라 `certification`이다**
> (`/discover`의 `certification_country`·`certification`, `/movie/{id}/release_dates`).
> 현재 우리에게 **등급 제한은 없다** — 청소년관람불가 작품은 이미 전부 들어오고 있다.

→ **6-3 ① · 6-4 1단계 · 6-6 `ADULT_CONTENT_NOT_ALLOWED` · 6-8 `include_adult=false` · 잔여 #21은
전부 현행 유지한다.** 스키마 변경 없음.

### 파급 항목

| 대상 | 변경 |
|---|---|
| `TmdbClient.discoverMovies` | ⚠️ `withGenres` 파라미터 추가 — **이번 확정분의 유일한 코드 변경** (잔여 #30) |
| `MovieSeedService.seedFromDiscover` | `genres` pass-through — 시그니처에 한 개 추가 |
| `AdminController` | `POST /api/admin/movies/seed/discover`에 `genres` 쿼리 파라미터 추가 |
| `movie-seed-runbook.md` | 프로필 v2 실행 커맨드 33회분으로 갱신 + 로그 보존 절차 (잔여 #26) — ✅ **완료** (2026-09-12) |
| 스키마 | **변경 없음.** ⚠️ **정오** — 현행은 v14가 아니라 **v15**다(2026-09-02 `review.rating` 제거, `cinemory_backup_v15.sql`). TMDB 동기화와 무관한 변경이라 이 문서가 v14에서 멈춰 있었다. `v15-delta.sql`이 스키마 폴더에 없다는 점도 확인할 것 |
| 6-5 프로필 표 | 이 절로 대체 (파라미터 pass-through 원칙·`region=KR` 철회는 유효) |
...
| 26 | ~~**`movie-seed-runbook.md` 보강**~~ (6-7-b) — 이번 실행에서 **`seed.log`가 유실**돼 `SeedResult` 8건과 절단 원본 길이를 확인하지 못했다. PowerShell `*>`가 **덮어쓰기**라 시드 후 `bootRun` 재시작에 날아간다(`*>>`가 이어쓰기). 그 밖에 ① **검증 절이 로그 grep만** — SQL 4종(규모·길이 분포·`EXPLAIN`·국가 구성)이 빠졌고, `grep -c`만으로는 **6-7의 "절단은 값 패턴이 아니라 길이로 판정" 교훈이 반영 안 됨**, ② **박스오피스 2주는 #10 표본으로 부족**(고유 제목 20~30편), ③ **토큰 30분 만료가 40~50분 작업 중간에 반드시 발생**하는데 PowerShell `catch`가 401을 삼켜 **뒤쪽 호출이 조용히 실패**, ④ 실행 결과를 **6-7-b에 기록하라는 안내 부재**, ⑤ 로그 인코딩(PS 5.1 = UTF-16LE)이라 `grep`이 못 읽음 | ✅ **구현 완료** (2026-09-12) — v2 프로필(4종·33회)로 전면 갱신, 토큰 자동 재발급 헬퍼(`Invoke-Seed`/`seed`), `seed-result.log` 분리, SQL 검증 5종(§7) 추가 |
...
| 28 | **`character_name` v16 확장 여부**(구 "v15" — 2026-09-02 `review.rating` 제거가 v15를 가져가 번호가 밀렸다) **— `varchar(255)`로도 부족한 케이스 3건** (2026-08-27 resync 실측). 확장 후 절단이 29건 → **3건**으로 줄었으나 0은 아니다. 이번엔 **자르지 않은 원본 길이**가 로그에 남았다 — `tmdbId=35`에서 **300자·270자**, `tmdbId=9473`에서 **348자**. ⚠️ **v14의 상한 근거가 약했던 것이 드러났다** — *"실측 원본이 100자를 갓 넘는 수준이라 2.5배면 충분"* 이라 적었는데, 그 "실측 원본"은 **이미 100자로 잘린 값이라 진짜 길이를 알 수 없는 상태**였다. **상한에 걸린 값으로 상한을 정한 셈**이다(6-7이 예상한 다역·성우 유형은 맞았고 꼬리가 예상보다 길었다). **지금은 확장하지 않는다** — 186,717행 중 **3건(0.0016%)** 이고, 절단이 WARN과 함께 graceful하며, **348자짜리 배역명은 애초에 UI에서 전부 표시할 값이 아니다**(DB 상한보다 표시 정책이 먼저 걸린다). 무엇보다 **복구에 resync 30분이 또 든다.** `varchar(255)→(500)`은 utf8mb4에서 길이 접두사가 그대로 2바이트라 **여전히 `ALGORITHM=INSTANT`** 이므로 ALTER 자체는 싸다 — 비싼 건 재적재다 | **다음에 resync를 돌릴 일이 생길 때 함께** (단독으로는 값이 안 됨) |
| 29 | ~~**6-10 프로필 v2 실행 및 실측**~~ — 34회 호출(설계 33회 + 박스오피스 역방향 1). 프로필별 기여 편수(`SeedResult`)를 **좁은 것부터 돌려 계측**하고, 결과를 **6-7-c**로 기록한다. 확인할 것 — ① 「올드보이」 등 `vote_count` 2,000~6,000 구간이 실제로 들어오는가(프로필 2 `pages=400`의 검증), ② 한국 영화가 843편에서 얼마나 느는가(`minVotes` 30→10), ③ 장르별 최소 편수가 M3 입력으로 쓸 만한가(프로필 4의 존재 이유), ④ 분당 155.5건 외삽이 시드에도 맞는가 | ✅ **완료** (2026-09-12) — 12,750편·약 44분·429 0건. 규모는 추정(1.8만~2만) 대비 낮았으나(겹침 과소평가) 장르별 하한 달성으로 목적은 충족. 상세는 **6-7-c** |
| 30 | ~~⚠️ **`TmdbClient.discoverMovies`에 `withGenres` 추가**~~ (6-10) — 프로필 4(장르 보강)의 **유일한 선행 코드 변경**이다. `queryParamIfPresent` 기존 패턴 그대로 null이면 붙이지 않고, `MovieSeedService.seedFromDiscover` → `AdminController`의 `genres` 쿼리 파라미터까지 3계층 pass-through(2026-08-24에 `lang`/`minVotes`/`sortBy`/`year`로 했던 것과 동일). **콤마=AND, 파이프=OR**에 주의 — 복수 장르를 콤마로 이으면 교집합이 되어 결과가 급감한다 | ✅ **구현 완료** (2026-09-12) |
| 31 | **`/movie/changes` 증분 동기화 검토** (6-10 Tier 3) — `resync` 전량이 4,609편에서 29분 30초였다. ⚠️ **재추정(2026-09-12)** — 적재가 12,750편이므로 **약 1시간 22분**이다(2만편 가정의 "2시간 10분"은 폐기). **시급성이 그만큼 낮아졌다** — `resync(fromId, limit)` 분할 실행으로 감당되는 범위다. `vote_average`가 계속 변해 **상시 필요한 작업**(6-9)인데 주기 실행 부담이 4.3배가 된다. TMDB `/movie/changes`는 **변경된 ID만** 주므로 비용이 DB 크기가 아니라 **변경량(일 수천 건)에 비례**한다. 다만 지금은 `resync(fromId, limit)`로 분할 실행이 가능해 급하지 않다 | **resync 주기 실행을 실제로 걸 때** || 32 | **`seed.log` 로그 레벨 점검** (6-7-c) — v2 시드 한 번에 서버 로그가 **343,795,238바이트(약 344MB)** 가 됐다. `SeedResult`는 `seed-result.log`로 분리돼 있어 계측은 무사했으나(잔여 #26 효과), 이 크기면 ① 절단·폴백 `grep` 검증이 느리고 ② 다음 확대에서 비례해 커지며 ③ 리포지토리 작업 중 실수로 커밋될 위험이 있다(`.gitignore` 확인). `show-sql`/Hibernate DEBUG가 켜져 있는지부터 볼 것 — 시드는 영화당 20여 개 INSERT라 SQL 로깅이 켜져 있으면 로그가 데이터보다 커진다 | **다음 시드 전** (검증 비용이 실제로 문제가 될 때) |
...
| 날짜 | 내용 |
|---|---|
| 2026-09-12 | **계측 정정 3건 기록 (6-7-c) — 6-10의 예측 모델이 틀렸다. 문서 수정만이며 코드·스키마 변경 없음.** ① ⚠️ **소요 예측이 2시간 → 실측 44분 4초.** 원인은 **v14 resync 처리율(분당 155.5건)을 시드에 그대로 외삽한 것**이다. 그 값은 *"영화 1편 = TMDB 왕복 1회 + 매핑 4종 삭제·재삽입"* 의 처리율인데, 시드에서는 **중복분이 `existsByTmdbId` 사전 필터에 걸려 detail 호출조차 하지 않는다.** 20,082 슬롯 중 실제 왕복은 8,109건뿐이었다 — **슬롯 기준 분당 456, 순 신규 기준 분당 184건**이고, **시드(184)가 resync(155.5)보다 오히려 빠르다**(신규 INSERT가 "벌크 삭제 → 재삽입 + dirty checking"보다 가볍다). 앞으로 **시드 예측에는 `184/분 × 예상 순 신규`, resync 예측에만 155.5/분**을 쓴다. ② **프로필 ④(`pages=400`) 단일 호출도 약 1시간 예상 → 17분 9초.** 같은 원인이며, 런북이 이 호출을 *"실패하면 처음부터라 맨 마지막에"* 배치한 판단 자체는 유효하다(리스크 근거가 소요 시간이 아니라 재개 불가였으므로). ③ ⚠️ **중복 손실 가정 7~16% → 실측 59.6%**(20,082 슬롯 중 `alreadyExists` 11,973). 원인은 **baseline 4,641편의 성격** — v1이 `vote_count.desc` 상위 위주라 같은 정렬축을 쓰는 프로필 ④(56.3%)·②(72.3%)가 구조적으로 그 위를 덮는다. 정렬축이 다른 ③(`popularity.desc`)은 52.3%로 상대적으로 낮았다. **이 59.6%를 다음에 그대로 쓰면 안 된다** — *"baseline이 이미 같은 정렬축의 상위를 덮고 있을 때"* 의 값이고, **6-7이 60편 표본에서 경고한 표본 편향과 같은 유형의 함정**이다. **파급** — `resync` 전량 재추정 **약 2시간 10분 → 약 1시간 22분**(12,750 ÷ 155.5; resync는 사전 필터가 없어 155.5가 그대로 유효하다), 이에 따라 **잔여 #31(`/movie/changes` 증분)의 시급성 하향** — `resync(fromId, limit)` 분할 실행으로 감당된다. **전량 미러링 기각은 불변** — 시드 처리율로 다시 계산해도 140만편은 6.3일이 아니라 **약 5.3일**이고, 6-10의 기각 사유 3건 중 처리율에 기대는 것은 하나뿐이다(나머지는 M3 인프라 선점과 품질 저하). ④ **잔여 #32 신설** — `seed.log`가 한 번의 시드로 **약 344MB**가 됐다(SQL/DEBUG 레벨 의심). `SeedResult` 분리(잔여 #26) 덕에 계측은 무사했으나 `grep` 검증 비용과 커밋 사고 위험이 있다. ⑤ ⚠️ **스키마 버전 정오** — 이 문서가 v14에서 멈춰 있었다. **현행은 v15**(2026-09-02 `review.rating` 제거, `cinemory_backup_v15.sql`)이며 TMDB 동기화와 무관한 변경이라 기록되지 않았다. 따라서 **잔여 #28의 `character_name` 확장은 v15가 아니라 v16**이고, 6-10 파급 항목의 *"v14 그대로"* 도 정정했다. **`v15-delta.sql`이 스키마 폴더에 없다**(v10~v14는 전부 존재) — 델타 파일 누락인지 다른 문서에 기록됐는지 확인이 필요하다. ⑥ **6-10 실행 순서 절의 "총 36회"를 34회로 정정**(discover 33 + 박스오피스 역방향 1) |
| 2026-09-12 | **6-7-c 신설 — 6-10 프로필 v2 실행 완료, 잔여 #29 종결. `movie` 4,609 → 12,750편.** `movie-seed-runbook.md` v2 그대로 34회(박스오피스 역방향 1 + discover 33) 호출, 00:43~01:28(약 44분), 429 0건. **프로필별 순 기여** — 박스오피스 역방향 11, 한국 영화 732, 장르 보강 1,579(19회 합), 최근작 2,288(12회 합), 전역 인지도 3,499. **확인사항 3종 전부 충족** — ① `vote_count` 2,000~6,000 구간 1,940편(「올드보이」 10,127표로 실제 포함 확인, 6-5의 `pages` 커트라인 문제 해소 재확인), ② 한국 영화 843→1,595편(minVotes 30→10 효과), ③ 장르 19종 전부 300편 이상(서부 313 최저) — 프로필②가 소외 장르(다큐·TV영화·서부·음악)에서 가장 크게 기여했고 baseline에 이미 있던 액션·드라마 등은 `matched=0`이었다. ⚠️ **수확 재판정 — 추정 1.8만~2만편 대비 12,750편(약 65~70%).** 원인은 프로필 간 겹침 과소평가 — baseline(v1)이 이미 전역 상위권 위주라 ④(전역, `alreadyExists` 56%)·②(장르, 대형 장르 대부분 `alreadyExists=300` 전량)의 실질 신규 기여가 예상보다 작았다. **재작업 불필요로 판단** — 목표가 "정확히 2만편"이 아니라 "장르별 M3 표본 확보"였고 확인사항 ③에서 달성됐다. **잔여 #28 재확인** — 절단 여전히 3편 수준(0.024%), 확장 보류 판정 유지. **잔여 #10은 유보** — 표본(280건, 58건 미매칭)이 여러 세션 누적이라 6-7-b(90.7%)와 단순 비교 불가, 재매칭 배치 실행 후 재확인 필요(신규 잔여 등록 없이 다음 점검에 포함). **검증** — DB 직접 쿼리(`movie`/`movie_actor`/`person`/`movie_country`/`movie_genre`/`box_office_record` 집계), `seed-result.log`(`SeedResult` 34건 전수), `seed.log`(절단 WARN 3건) 대조 |
| 2026-09-12 | **잔여 #26 종결 — `movie-seed-runbook.md`를 6-10 프로필 v2 기준으로 전면 갱신.** 6-7-b가 지적한 5가지 결함을 모두 해소했다. **변경** — ① 프로필 3종·7회(v1) → **4종·33회**(한국 1·장르 보강 19·최근작 12·전역 1)로 재구성, 장르 보강은 TMDB 장르 ID 19종을 배열로 순회. ② PowerShell `bootRun *>`(덮어쓰기) → **`*>>`**로 교체. ③ **토큰 자동 재발급 헬퍼**(`Invoke-Seed`/bash `seed`) 신설 — 401을 만나면 `Reissue-Token`/`reissue`를 호출해 1회 재시도 후 계속 진행(기존엔 `catch`가 401을 삼켜 뒤쪽 호출이 조용히 실패했다). ④ API 응답을 **`seed-result.log`로 로그 분리**(`seed.log`는 서버 로그 전용) — `SeedResult` 33건이 잔여 #29의 유일한 근거이므로 유실 방지가 핵심이다. ⑤ **검증 절 확장** — 기존 `grep` 4종에 더해 SQL 5종(§7: `vote_count` 2,000~6,000 구간·국가 구성·장르별 편수·전체 규모·v13 컬럼)을 추가해 "절단은 값 패턴이 아니라 길이로 판정"(6-7) 원칙을 실제로 반영했다. **의도적으로 하지 않은 것** — 프로필 ④(`pages=400`, 단일 호출 약 1시간)는 시작 페이지 재개 파라미터가 없어 실패 시 처음부터이므로 **맨 마지막에 배치**했다(나머지 32회 결과는 이미 확보된 상태로 유지). **검증 방식** — 문서 변경만이라 컴파일 대상 없음. 실제 실행(운영자가 33회를 직접 호출하는 것)은 별도 |
| 2026-09-12 | **잔여 #30 종결 — `TmdbClient.discoverMovies`에 `withGenres` 파라미터 추가(6-10 프로필 4 선행 코드 변경).** 2026-09-11 확정분 그대로 구현했으며 설계 변경은 없다. **변경** — `TmdbClient.discoverMovies(int page, String originalLanguage, Integer voteCountGte, String sortBy, Integer year)` → 끝에 `String withGenres` 추가, 기존 4개와 동일하게 `queryParamIfPresent("with_genres", ...)`로 null이면 붙이지 않는다. `MovieSeedService.seedFromDiscover`에 `genres` 파라미터를 추가해 그대로 pass-through, `AdminController`의 `POST /api/admin/movies/seed/discover`에 `genres` 쿼리 파라미터(`required=false`) 추가 — 2026-08-24에 `lang`/`minVotes`/`sortBy`/`year`로 했던 3계층 pass-through와 동일한 패턴이다. 스키마 변경 없음(v14 그대로). **검증** — `./gradlew compileJava` 통과. 프로필 v2 실행·실측(잔여 #29)은 이 코드 반영 이후 별도 세션 |
| 2026-09-11 | **6-10 신설 — 적재 규모 확대 확정(4,609편 → 약 2만편). 설계 논의만 있었고 코드 변경은 없다.** ① **전량 미러링(약 140만편) 기각** — v14 resync 실측(4,587건 / 29분 30초 = **분당 155.5건**)을 외삽하면 최초 적재도 `resync` 1회도 **약 6.3일**이다. 6-9가 *"`resync`는 선택이 아니라 필수"* 라고 확정한 이상 **1회 6일짜리 상시 작업은 성립하지 않는다.** 여기에 M3 임베딩이 4.2GB가 되어 **추천 설계가 확정되기도 전에 외부 벡터 DB를 전제로 못 박게 되고**, 늘어나는 135만편 대부분이 `vote_count = 0`이라 검색·추천 품질이 **오히려 나빠진다**(결국 쿼리마다 하한을 다시 붙이게 되는데 그러면 안 넣은 것과 같다). ② ⚠️ **6-5의 진단을 뒤집었다 — 실질 커트라인은 `vote_count.gte`가 아니라 `pages`였다.** 프로필 2는 `minVotes=300` + `sort_by=vote_count.desc`인데, **정렬이 이미 표 많은 순이라 2,000번째 영화의 표 수가 300보다 한참 높다.** 즉 **하한 300은 단 한 편도 거르지 않았고**, 실제 커트라인은 `pages × 20 = 상위 2,000위`(대략 `vote_count` 5,000~6,000)였다. 「올드보이」(약 4,500표)가 빠지고 「미드소마」(약 7,000표)가 아슬아슬하게 들어온 이유가 이것이다. 6-5가 *"임계값(30/300/100)은 첫 결과를 보고 조정할 값"* 이라고 지목한 것이 **절반만 맞았다** — `sort_by=popularity.desc`인 프로필 3에서만 하한이 실제로 작동한다. ③ **프로필 v2 확정** — 1(한국) `minVotes` 30→**10**·`pages` 50→**150**(6-7-b에서 843편으로 목표 1,000 미달), 2(전역) `pages` 100→**400**(⚠️ TMDB 페이지 상한 500, 450 넘지 말 것), 3(최근작) 연도 2021~2025→**2015~2026**·하한 100→**30**(2026년작은 아직 표가 안 쌓여 100이면 전멸), 4(**장르 보강, 신설**) `genres=<id>` × 19종 × 15p — 프로필 2가 전역 `vote_count.desc`라 **다큐·애니·서부·음악은 장르 내 대표작조차 상위 8,000에 못 들고**, 「배경」절이 밝힌 대로 **M3 추천 입력이 `movie_genre` 가중치**이므로 장르 표본이 비면 그 장르 추천이 성립하지 않는다. 합계 21,500 요청 → 중복 제거 후 **약 1.8만~2만편**. ④ **온디맨드(Tier 2)가 커버리지 문제를 이미 풀고 있음을 명문화** — *"기록하려는 영화가 DB에 없다"* 는 Tier 1을 키워서가 아니라 6-5 온디맨드 절 + 6-8 `suggestions`가 푼다(저장 2만편 / 체감 140만편). Tier 1의 목적은 커버리지가 아니라 **검색 첫 화면·박스오피스 매칭·M3 후보군**으로 재정의했다. ⑤ **실행 순서 기준 변경** — 6-5의 *"박스오피스 먼저"* 근거(잔여 #10)는 6-7-b에서 90.7%로 종결됐으므로, 이번엔 **좁은 프로필부터**가 기준이다(넓은 것을 먼저 돌리면 나머지가 전부 `alreadyExists`로 빠져 프로필별 기여도를 계측할 수 없다). ⑥ ⚠️ **`adult` 필터 폐기안을 기각했다** — *"성인 영화에도 좋은 작품이 많다"* 는 전제로 6-3 ① 폐기가 제안됐으나, TMDB 영화 기여 지침서가 `adult`를 **하드코어 포르노 전용**으로 규정한다. 18금 에로틱 영화는 adult로 표시하지 않고(「그레이의 50가지 그림자」·「님포매니악」이 지침서의 명시적 반례), 하드코어 장면이 있어도 일반 영화로 개봉·평론된 작품은 예외이며(「9 Songs」 등), 포르노 다큐도 일반 영화다. **「올드보이」·「미드소마」는 `adult:false`이고 원래부터 수집 대상**이었으므로 **필터를 지워도 그 영화들은 여전히 안 들어왔을 것**이다 — 원인은 ②의 `pages` 커트라인이었다. **6-3 ① · 6-4 1단계 · 6-6 `ADULT_CONTENT_NOT_ALLOWED` · 6-8 `include_adult=false` · 잔여 #21 전부 현행 유지.** 연령 등급으로 통제할 축은 `adult`가 아니라 `certification`(`/discover`의 `certification_country`·`certification`, `/movie/{id}/release_dates`)이며, **현재 등급 제한은 없어 청소년관람불가 작품은 이미 전부 들어오고 있다.** ⑦ **잔여 재판정** — #8(`movie_actor` 인덱스) **재측정 불요**(6-7-b가 79배 증가에도 `EXPLAIN rows=141` 불변 확인, 4.3배 추가는 판정을 못 뒤집는다), #20(한글 FULLTEXT) **아직 불요, 트리거 재설정 — `movie` 10만행 돌파 또는 검색 p95 300ms 초과**, #18(다중 인스턴스) **위험 상승**(시드가 2시간짜리가 되어 `AtomicBoolean` 노출 구간이 길어짐), #26(런북) **선행 조건으로 승격**(36회 호출이라 `seed.log` 유실이 재발하면 계측 자체가 무산 — PowerShell `*>`는 덮어쓰기이므로 `*>>` 또는 프로필별 파일 분리). **신규 등록 — 잔여 #29(프로필 v2 실행·실측 → 6-7-c) · #30(`withGenres` 파라미터, 유일한 코드 변경) · #31(`/movie/changes` 증분 검토).** **스키마 변경 없음**(v14 그대로) |
