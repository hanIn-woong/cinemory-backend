...
public class BoxOfficeSyncService {
     ...
     *
     * <p><b>한계</b>: 기획 단계의 매칭 전략은 "한글 제목 + 개봉연도"였으나, {@code box_office_record}에
     * KOFIC의 {@code openDt}(개봉일)를 저장하는 컬럼이 없어 개봉연도로 후보를 좁힐 수 없다.
     * 따라서 제목이 정확히 일치하고 <b>후보가 유일할 때만</b> 연결하고, 2건 이상이면 오매칭을 피해
     * 건너뛴다. 정확도를 더 올리려면 스키마에 {@code open_date} 컬럼 추가(v9)가 선행돼야 한다.
     *
